...
def show_all():
    ...
    for idx in idx_list:
        article_id = cur.execute(
            "SELECT Article_Id FROM sections WHERE Section_Id = (?)", (idx,)
        ).fetchone()[0]
        abstract = cur.execute(
            "SELECT Abstract FROM articles WHERE Article_Id = (?)", (article_id,)
        ).fetchone()[0]
        title = cur.execute(
            "SELECT Title FROM articles WHERE Article_Id = (?)", (article_id,)
        ).fetchone()[0]
        published_date = cur.execute(
            "SELECT Published_Date FROM articles WHERE Article_Id = (?)", (article_id,)
        ).fetchone()[0]
        authors = cur.execute(
            "SELECT Authors FROM articles WHERE Article_Id = (?)", (article_id,)
        ).fetchone()[0]
        journal = cur.execute(
            "SELECT Journal FROM articles WHERE Article_Id = (?)", (article_id,)
        ).fetchone()[0]
        url = cur.execute(
            "SELECT Url FROM articles WHERE Article_Id = (?)", (article_id,)
        ).fetchone()[0]

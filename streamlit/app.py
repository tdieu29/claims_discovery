...
from colbert.retrieve import retrieve
...
def main(args):
    ...
    rationales_selected, predicted_labels = search(query, ar_model, checkpoint, ss_model, lp_model)
...
def search(query, ar_model, checkpoint, ss_model, lp_model):
    ...
    if st.button("Search"):   
        with st.spinner('Searching...'): 
            ...
            abstracts_retrieved = retrieve(query_dict, ar_model, checkpoint)
...
def retrieve_info(cur, id):
    ...
    abstract = cur.execute("SELECT Abstract FROM articles WHERE Article_Id = (?)", (id,)).fetchone()[0] 
    title =  cur.execute("SELECT Title FROM articles WHERE Article_Id = (?)", (id,)).fetchone()[0]  
    published_date = cur.execute("SELECT Published_Date FROM articles WHERE Article_Id = (?)", (id,)).fetchone()[0] 
    authors = cur.execute("SELECT Authors FROM articles WHERE Article_Id = (?)", (id,)).fetchone()[0] 
    journal = cur.execute("SELECT Journal FROM articles WHERE Article_Id = (?)", (id,)).fetchone()[0] 
    url = cur.execute("SELECT Url FROM articles WHERE Article_Id = (?)", (id,)).fetchone()[0]  
...
def display_results(cur, result_list, rationales_selected):
    for i, abstract_id in enumerate(result_list):
        abstract, title, published_date, authors, journal, url = retrieve_info(cur, abstract_id)

        if published_date == '' or published_date == None:
        ...
        for idx in sentence_indexes:
            sentence = cur.execute("SELECT Sentence FROM sentences WHERE Article_Id = (?) AND Sentence_Index = (?)", (abstract_id, idx)).fetchone()[0]
            st.write(f'-  {sentence}')

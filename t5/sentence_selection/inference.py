...
def rationale_selection(query, abstracts_retrieved, SS_MonoT5_model):
    ...
    for id in top_50_article_ids:
        abstract_text = cur.execute("SELECT Abstract FROM articles WHERE Article_Id = (?)", (id,)).fetchone()[0] 
        ...
        for i in range(len(sentences)):
            ...
            key = str(id) + '#' + str(i)
            all_sentences[key] = {'score': None, 
                                  'text': sentences[i].strip()}
    
    # Score each sentence in each abstract
    SS_scored_documents = SS_MonoT5_model.rescore(queries=all_queries, texts=all_documents)
    ...
    for i in range(len(SS_scored_documents)):
        assert SS_scored_documents[i].text == all_documents[i].text == all_sentences[keys[i]]['text']
        assert SS_scored_documents[i].score == None

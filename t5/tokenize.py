...
TokenizerReturnType = Mapping[
    str, Union[torch.Tensor, List[int], List[List[int]], List[List[str]]]
]
...
class TokenizerEncodeMixin:
    ...
    def encode(self, strings: List[str]) -> TokenizerReturnType:
        assert (
            self.tokenizer and self.tokenizer_kwargs is not None
...
class QueryDocumentBatchTokenizer(TokenizerEncodeMixin):
    ...
    def traverse_query_document(
        self, batch_input: QueryDocumentBatch
    ) -> Iterable[QueryDocumentBatch]:
        for batch_idx in range(0, len(batch_input), self.batch_size):
            ...
            outputs = self.encode(
                [
                    self.pattern.format(query=queries[i].text, document=docs[i].text)

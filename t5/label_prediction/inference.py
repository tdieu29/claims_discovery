...
def label_prediction(query, rationales_selected, LP_MonoT5_model):
    ...
    for abstract_id in rationales_selected:
        sent_indexes = sorted(
            rationales_selected[abstract_id]
        )  # Indexes of rationale sentences in abstract
        ...
        evidence = " ".join(
            [
                f"sentence{idx + 1}: " + sentences[index].strip()
                for idx, index in enumerate(sent_indexes)
            ]
        )
    ...
    for i in range(len(LP_scored_documents)):
        ...
        abstract = cur.execute(
            "SELECT Abstract FROM articles WHERE Article_Id = (?)", (doc_ids[i],)
        ).fetchone()[0]
        ...
        length = len(
            first_rationale_sent
        )  # Length of the first rationale sentence in this abstract

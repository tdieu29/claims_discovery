...
class Execute(object): 
    ...
    def entryDates(indir, entryfile):
        ...
        dates = {}
        count = 0
        with open(os.path.join(indir, "metadata.csv"), encoding = "utf8", mode="r") as csvfile: 
            for row in csv.DictReader(csvfile): 
                # Lookup hash 
    ...
    def process(row):
        ...
        metadata = (row['cord_uid'], row["source_x"], row["title"], row["abstract"], p_date, row["authors"], row["journal"], Execute.getUrl(row)) 
    ...
    def run(indir, outdir, entryfile, merge_url):
        ...
        with Pool(os.cpu_count()) as pool:
            for article in pool.imap(Execute.process, Execute.stream(indir, dates, merge_uids), 100):

...
def index_faiss(args):
    ...
    if args.sample is not None:
        ...
        print_message(f"#> Training with {round(args.sample * 100.0, 1)}% of *all* embeddings.")
    ...
    for slice_idx, part_offset in enumerate(range(0, len(parts), num_parts_per_slice)):
        ...
        index = prepare_faiss_index(slice_samples_paths, args.partitions, args.nprobe, args.sample)

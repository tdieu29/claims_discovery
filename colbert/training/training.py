...
from colbert.training.utils import print_progress, manage_checkpoints_pretrain, manage_checkpoints
...
def train(args):
    ...
    if args.resume: 
        if args.pretrain:
            ...
            reader.skip_to_batch(start_batch_idx, args.bsize, position_nf, position_bioS, switch_to_bioS, count)
    ...
    for epoch_idx in range(args.num_epochs): 
        for batch_idx, BatchSteps in zip(range(start_batch_idx, args.maxsteps), reader):
            ...
            for queries, passages in batch:
                ...
                _, psg_attn_mask = passages  # shape = torch.Size([batch size, doc_seq_len])
                
                with amp.context():
                    scores = colbert(queries, passages, psg_attn_mask).view(2, -1).permute(1, 0)
            ...
            if (batch_idx + 1) % args.accumsteps == 0: 
                ...
                if not args.pretrain:
                    print('position_nf: ', position_nf, '\t\t|\t\t', 'position_bioS: ', position_bioS)
                    print('switch_to_bioS: ', switch_to_bioS, '\t\t|\t\t', 'count: ', count)
            ...
            if args.pretrain: 
                manage_checkpoints_pretrain(epoch_idx, batch_idx+1, train_loss, colbert, optimizer)
            else:
                manage_checkpoints(epoch_idx, batch_idx+1, train_loss, colbert, optimizer, position_nf, position_bioS, switch_to_bioS, count) 
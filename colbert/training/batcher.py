...
class Batcher():
    ...
    def _load_triples(self, path, name):
        ...
        for line in csvFile: 
            ...
            qid, pos, span_pos, neg, span_neg = str(qid), str(pos), int(span_pos), str(neg), int(span_neg)
    ...
    def __next__(self):
        ...
        if self.count == 16 and self.switch_to_bioS == False:
            self.switch_to_bioS = True # Switch to using BioASQ_Scifact 
            self.count = 0 # Reset count
        elif self.count == 16 and self.switch_to_bioS == True:
            self.switch_to_bioS = False # Switch to using NF Corpus
            self.count = 0 # Reset count

        if self.switch_to_bioS == False:
            offset, endpos = self.position_nf, min(self.position_nf + self.bsize, len(self.triples_nf)) 
        ...
        elif self.switch_to_bioS == True: 
            offset, endpos = self.position_bioS, min(self.position_bioS + self.bsize, len(self.triples_bioS)) 
        ...
        if self.position_nf == len(self.triples_nf) or self.position_bioS == len(self.triples_bioS): # NEED TO ITERATE OVER BIOS MORE THAN ONCE, SO FIX THIS LINE !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            self.stop = True
    ...
    def skip_to_batch(self, batch_idx, intended_batch_size, position_nf, position_bioS, switch_to_bioS, count):
